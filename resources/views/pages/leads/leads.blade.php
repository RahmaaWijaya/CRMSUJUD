                ...
                <div x-data="{ open: false }" class="relative inline-block text-left">
                    ...
                    <div x-show="open" @click.away="open = false" class="origin-top-right absolute right-0 mt-1 w-full rounded-b-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 focus:outline-none z-10" style="display: none;">
                        <div class="py-2" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
                            <!-- Create Lead -->
                            <a href="{{ route('create-lead') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" role="menuitem">Create Leads</a>
                            <!-- Import Leads -->
                            <a href="{{ route('import-leads')}}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" role="menuitem">Import Leads</a>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <div class="overflow-x-auto">
                            <table class="table-auto w-full">
                                ...
                                <tbody class="text-sm divide-y divide-gray-100 dark:divide-gray-700/60 ">
                                    ...
                                    <tr href="{{ route('lead-detail', $lead->leads_id) }}" class="cursor-pointer bg-gradient-to-br from-transparent to-transparent hover:from-violet-500/10 hover:via-violet-400/10 hover:to-violet-500/10">
                                        ...
                                        <td class="p-2 whitespace-nowrap">
                                            <a href="{{ route('lead-detail', $lead->leads_id) }}" class="text-left font-medium text-green-500">{{ $lead->no_tlp }}</a>
                                        </td>
                                        <td class="p-2 whitespace-nowrap">
                                            <a href="{{ route('lead-detail', $lead->leads_id) }}" class="text-center">{{ $lead->alamat }}</a>
                                        </td>
                                        <td class="p-2 whitespace-nowrap">
                                            <a href="{{ route('lead-detail', $lead->leads_id) }}" class="text-center">{{ $lead->status }}</a>
                                        </td>
                                        <td class="p-2 whitespace-nowrap">
                                            <a href="{{ route('lead-detail', $lead->leads_id) }}" class="text-center">{{ $lead->waktu->format('Y-m-d') }}</a>
                                        </td>
                                        <td class="p-2 whitespace-nowrap">
                                            <a href="{{ route('lead-detail', $lead->leads_id) }}" class="text-center">{{ $lead->waktu->format('H:i:s') }}</a>
                                        </td>
